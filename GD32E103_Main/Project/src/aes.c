...
static void ECB_Encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output)
{
...
static void ECB_Decrypt(const uint8_t* input, const uint8_t* key, uint8_t *output)
{
  // Copy input to output, and work in-memory on output
  memcpy(output, input, 16);
  state = (state_t*)output;
...
/*****************************************************************************/
/**
 * @brief	:  Ham ma hoa AES128
 * @param	:  input*, key*, output*, input length
 * @retval	:
 * @author	:	
 * @created	:	10/11/2014
 * @version	:
...
uint16_t AES_ECB_encrypt(uint8_t* input, const uint8_t* key, uint8_t* output, const uint32_t inputLength)
{
	...
	uint8_t numBlock = inputLength/16 + 1;	//PKCS7 mode: Neu length la boi cua block -> them extra block trong sau cung!
	
	PaddingNumber = 16 * numBlock;

	/* Add padding byte, value = PaddingNumber - length */
	for(i = 0; i < PaddingNumber; i++)
	{
		 if(i >= inputLength) input[i] = PaddingNumber - inputLength;
	}

	/* Encrypt block 16 */
	for(i = 0; i < PaddingNumber; i += 16) 
	{
		ECB_Encrypt(&input[i], key, &output[i]);
	}
...
void AES_ECB_decrypt(const uint8_t* input, const uint8_t* key, uint8_t *output, const uint32_t length)
{
	...
	{
		ECB_Decrypt(&input[i], key, &output[i]);
	}
}
