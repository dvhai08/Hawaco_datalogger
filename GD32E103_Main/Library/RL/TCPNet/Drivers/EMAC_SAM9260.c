 ...
 *----------------------------------------------------------------------------
 *      Name:    EMAC_SAM9260.C
 *      Purpose: Driver for Atmel AT91SAM9260 EMAC Ethernet Controller
 *      Rev.:    V4.54
 *----------------------------------------------------------------------------
...
static Buf_Desc Rx_Desc[NUM_RX_BUF] __at(0x200040);
static Buf_Desc Tx_Desc[NUM_TX_BUF] __at(0x200140);

/* EMAC local buffers must be 8-byte aligned. */
static U64 rx_buf[NUM_RX_BUF][ETH_RX_BUF_SIZE>>3] __at(0x300000);
static U64 tx_buf[NUM_TX_BUF][ETH_TX_BUF_SIZE>>3] __at(0x200148);

/* Note: TX and RX buffers and descriptors need to be allocated in internal
   RAM or any other memory slave with a short access time. The SDRAM refresh
   cycle or SDRAM bank opening might generate TX underrun (latency min. 960ns).
   If buffers and descriptors are located in SDRAM, the application crashes
   in Data Abort.

   Reference AT91SAM9260 Errata 
     44.2.3   EMAC
     44.2.3.1 TX underrun may occur in some cases                            */
...
static void write_PHY (U32 PhyReg, U16 Value) {
  ...
  pEMAC->EMAC_MAN = ((0x01<<30) | (2 << 16) | (1 << 28) |
...
static U16 read_PHY (U32 PhyReg) {
  ...
  pEMAC->EMAC_MAN = (0x01<<30) | (2 << 16) | (2 << 28) |
...
static void enable_MDI (void) {
  ...
  /* MDC = MCK/64 */
  pEMAC->EMAC_NCFGR |= AT91C_EMAC_CLK_HCLK_64;
}
