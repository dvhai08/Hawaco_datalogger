...
static void gsm_query_sms_buffer(void)
{
    ...
    {
        if (sms[cnt].need_to_send == 1 || sms[cnt].need_to_send == 2)
        {
            sms[cnt].retry_count++;
...
void gsm_manager_tick(void)
{
    ...
    case GSM_STATE_OK:
    {
        ...
        if (eeprom_cfg->io_enable.name.register_sim_status == 0 && strlen((char *)eeprom_cfg->phone) > 9 && eeprom_cfg->io_enable.name.warning && (ctx->status.total_sms_in_24_hour < eeprom_cfg->max_sms_1_day))
        {
            char msg[128];
            char *p = msg;
            rtc_date_time_t time;
        ...
        if (gsm_manager.state == GSM_STATE_OK) // gsm state maybe changed in gsm_query_sms_buffer task
        {
            ...
            if ((measure_input_sensor_data_availble() || m_retransmision_data_in_flash) && !ctx->status.enter_ota_update && m_post_failed == false)
            {
                enter_post = true;
            }

            if (enter_post == false && m_post_failed == false && m_retransmision_data_in_flash == NULL)
            {
            ...
            else // Enter http get
            {
                ...
                if (gsm_manager.state == GSM_STATE_OK)
                {
                    ...
                    if (GSM_NEED_ENTER_HTTP_GET() || ctx->status.enter_ota_update || (ctx->status.new_server && ctx->status.try_new_server))
                    {
    ...
    case GSM_STATE_HTTP_GET:
    {
        ...
        if (GSM_NEED_ENTER_HTTP_GET())
        {
            ...
            if (!sys_ctx()->status.enter_ota_update && !ctx->status.try_new_server)
            {
...
bool need_update_time(uint32_t new_counter, uint32_t current_counter)
{
    ...
    if (m_last_time_update == 0 || (current_counter - m_last_time_update >= (uint32_t)(3600 * 12)) || ((new_counter >= current_counter) && (new_counter - current_counter >= (uint32_t)60)) || ((new_counter < current_counter) && (current_counter - new_counter >= (uint32_t)60)))
    {
...
// Reg0_1:1234, Reg0_2:4567, Reg1_0:12345
    ...
    for (uint32_t index = 0; index < RS485_MAX_SLAVE_ON_BUS; index++)
    {
        ...
        {
            if (msg->rs485[index].sub_register[sub_idx].read_ok && msg->rs485[index].sub_register[sub_idx].data_type.name.valid)
            {
                uint32_t tmp_len = total_length;
                total_length += sprintf((char *)(ptr + total_length), "\"Rg%u_%u\":", index + 1, sub_idx + 1);
                if (msg->rs485[index].sub_register[sub_idx].data_type.name.type == RS485_DATA_TYPE_INT16 || msg->rs485[index].sub_register[sub_idx].data_type.name.type == RS485_DATA_TYPE_INT32)
                {
...
// Analog input
    ...
    case GSM_HTTP_GET_EVENT_FINISH_SUCCESS:
    {
        ...
        if (ctx->status.last_state_is_disconnect)
        {
            ...
            if (strlen((char *)eeprom_cfg->phone) > 9 && eeprom_cfg->io_enable.name.warning && (ctx->status.total_sms_in_24_hour < eeprom_cfg->max_sms_1_day))
            {
                char msg[128];
                char *p = msg;
                rtc_date_time_t time;
